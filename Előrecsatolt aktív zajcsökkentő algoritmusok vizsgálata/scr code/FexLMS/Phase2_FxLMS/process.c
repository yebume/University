...
float beta = 1;
//float mu2 		= 0.00000005;		//---------------------------------------------------------------------------------------------mu2
//float mu2def	= 0.00000005;		//---------------------------------------------------------------------------------------------mu2
								...
								//101-es szûrõ, 100-as mélység
									...
									//zaj: 0.01;
//sin 0.000005;
// 0.00001 ha 121 eható
// 0.00001;
//0.0000000004
float mu2 		= 0.000008;		//---------------------------------------------------------------------------------------------mu2
float mu2def	= 0.000008;		//---------------------------------------------------------------------------------------------mu2
float mu2_0     = 0.0001;		//---------------------------------------------------------------------------------------------mu2_0
//white noise 0.00001
...
//	****************   phase 2  ****************
	...
	if(fxlmsFlag == 1){			//sw2
		...
		inputRfil[mh] = inputRfil_n;		//done
		if(mh==0){
			inputRfil[Nh] = inputRfil_n;
		}
			
		inputH_n = conv_asm( coefs_h, &(inputRfil[mh]), inputRfil, Nh);				// filtering with h
		inputH[m2] = inputH_n;
		if (m2==0)
			inputH[N2] = inputH_n;
		...
		if (mh==0)
			err2arr[Nh] = inLdata;
		float err2_new = conv_asm( coefs_h, &(err2arr[mh]), err2arr, Nh);
		k = 0;
		k_input = m2;
		...
		for(k; k<N2; k++){
		    ...
		    coefs_filter_2[k] = coefs_filter_2[k] + mu2*err2_new*inputH[k_input%N2];		//done
			k_input++;
		}	
