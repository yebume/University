function [e,w,yk] = myELMS(M, mu, A2_k, A2_i, delay, s, d,start)
%OUTPUT: [e,w]
...
state_e = 0;
memory_e = 0;
s_delayed = [zeros(delay,1); s];

    for k = 1:L     % k: futó IDŐváltózó
        x(1)   = s(k);                  % FIR tartalmazza
                z(1) = s_delayed(k);
                %[z(1), state_z, memory_z] = myIIR4one(1, 1, state_z, memory_z, s(k));    %késleltetés
        y      = w' * x;                % FIR tartalmazza
            [y, state_y, memory_y] = myIIR4one(A2_k, 1, state_y, memory_y, y);
        e(k)   = d(k)  - y;
            [e_z(k), state_e, memory_e] = myIIR4one(A2_i, 1, state_e, memory_e, e(k));
